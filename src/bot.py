...
class GameAssistBot(commands.Bot):
    def __init__(self, settings: Settings, cache: SQLiteCache, sources: SourceRegistry) -> None:
        ...
        self._reputation_recovery_guilds: set[int] = set()
    ...
    def track_reputation_verification(self, coroutine) -> None:
        task = asyncio.create_task(coroutine)
        self._reputation_verification_tasks.add(task)
        task.add_done_callback(self.finish_reputation_verification_task)
    ...
    async def setup_hook(self) -> None:
        ...
        if (self.settings.award_test_guild_id
                and self.settings.award_test_guild_id != self.settings.discord_guild_id):
    ...
    async def sync_sc_companion_category_guides(self, guild: discord.Guild) -> None:
        ...
        if guild.me is None:
            return
        peep_category_names = {SC_COMPANION_CATEGORY_NAME, "SC Companion", VISITOR_CATEGORY_NAME}
        channels = [
    ...
    async def _is_shared_setup_channel(self, channel: discord.abc.GuildChannel) -> bool:
        ...
        automatic_names.update({SC_COMPANION_CATEGORY_NAME, "SC Companion", "marketplace"})
    ...
    async def notify_reputation_applicant(self, applicant_id: int, text: str) -> None:
        user = self.get_user(applicant_id)
        if user is None:
            with suppress(discord.NotFound, discord.HTTPException):
                user = await self.fetch_user(applicant_id)
        if user is None:
            return
        with suppress(discord.Forbidden, discord.HTTPException):
            await user.send(text)

    async def verify_queued_reputation_submission(
        self,
        *,
        ...
        """Finish OCR for a new or restart-recovered queue message."""
        ...
        await self.notify_reputation_applicant(
            ...
            "It is now waiting for a reviewer; this is not a denial.",
        )

    async def restore_pending_reputation_reviews(
        self, guild: discord.Guild, review_queue: discord.TextChannel
    ) -> None:
        """Resume queue items left pending by a worker restart."""
        if guild.id in self._reputation_recovery_guilds:
            return
        self._reputation_recovery_guilds.add(guild.id)
        if self.user is None:
            return
        async for message in review_queue.history(limit=100):
            if message.author.id != self.user.id or not message.embeds:
                continue
            embed = message.embeds[0]
            fields = {field.name: field.value for field in embed.fields}
            verification_state = fields.get("Automatic verification", "")
            ...
            image_bytes = b""
            if message.attachments:
                with suppress(discord.NotFound, discord.HTTPException, OSError):
                    image_bytes = await message.attachments[0].read(use_cached=True)
            self.track_reputation_verification(self.verify_queued_reputation_submission(
                message=message,
                guild_id=guild.id,
    ...
    async def review_reputation_application(
        ...
        await self.notify_reputation_applicant(
            member.id,
            f"{'✅' if approved else '❌'} Your **{giver} — {level}** reputation submission "
            f"was {status.lower()} by a reviewer in **{guild.name}**.",
        )
    ...
    async def ensure_reputation_submission_channels(self, guild: discord.Guild) -> None:
        ...
        await self.restore_pending_reputation_reviews(guild, review_queue)
...
class AwardManagerRoleSelect(discord.ui.RoleSelect):
    def __init__(self) -> None:
        super().__init__(placeholder="Choose the award manager role", min_values=1, max_values=1, row=1)
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if interaction.guild.owner_id != interaction.user.id:
            await interaction.response.send_message("Only the server owner can change the award manager role.", ephemeral=True)
            return
        settings = await bot.cache.award_settings(interaction.guild.id)
        await bot.cache.save_award_settings(
            interaction.guild.id, settings["enabled"], self.values[0].id, interaction.user.id,
            settings.get("announcement_channel_id"),
        )
        await interaction.response.send_message(f"Award managers set to {self.values[0].mention}.", ephemeral=True)


class AwardAnnouncementChannelSelect(discord.ui.ChannelSelect):
    def __init__(self) -> None:
        super().__init__(placeholder="Choose the award announcement channel", min_values=1, max_values=1,
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if not isinstance(bot, GameAssistBot) or interaction.guild is None or not await _can_manage_awards(interaction, bot):
            await interaction.response.send_message("Only the server owner or award manager can set this channel.", ephemeral=True)
            return
        settings = await bot.cache.award_settings(interaction.guild.id)
        ...
        await interaction.response.send_message(f"Award announcements will be posted in {channel.mention}.", ephemeral=True)


class AwardAdminView(discord.ui.View):
    def __init__(self, modules: dict[str, dict[str, object]]) -> None:
        super().__init__(timeout=900)
        self.modules = modules
        self.add_item(AwardManagerRoleSelect())
        self.add_item(AwardAnnouncementChannelSelect())
    ...
    async def toggle(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        if not isinstance(bot, GameAssistBot) or interaction.guild is None or not _award_test_guild(interaction, bot):
            ...
            return
        if interaction.guild.owner_id != interaction.user.id:
            await interaction.response.send_message("Only the server owner can enable or disable awards.", ephemeral=True)
            return
        settings = await bot.cache.award_settings(interaction.guild.id)
        await bot.cache.save_award_settings(
            interaction.guild.id, not settings["enabled"], settings.get("manager_role_id"), interaction.user.id,
            settings.get("announcement_channel_id"),
        )
        await interaction.response.send_message(
    ...
    async def back(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        del button
        await interaction.response.edit_message(
            embed=build_native_admin_embed(interaction.guild, self.modules), view=NativeAdminView(self.modules)
        )


class NativeAdminView(discord.ui.View):
    def __init__(self, modules: dict[str, dict[str, object]], setup_mode: str = "manual",
                 awards_available: bool = False) -> None:
        super().__init__(timeout=900)
        self.modules = modules
        ...
        self.add_item(guide)
        if awards_available:
            awards = discord.ui.Button(label="Award Setup", style=discord.ButtonStyle.secondary, emoji="🏆")
            awards.callback = self.show_awards
            self.add_item(awards)
    ...
    async def show_awards(self, interaction: discord.Interaction) -> None:
        bot = interaction.client
        if not isinstance(bot, GameAssistBot) or interaction.guild is None or not _award_test_guild(interaction, bot):
            await interaction.response.send_message("Awards are currently limited to the SC Companion testing Discord.", ephemeral=True)
            return
        settings = await bot.cache.award_settings(interaction.guild.id)
        embed = discord.Embed(
        ...
        embed.add_field(name="Manager role", value=(f"<@&{settings['manager_role_id']}>"
                                                       if settings.get("manager_role_id") else "Server owner only"), inline=True)
        embed.add_field(name="Announcement channel", value=(f"<#{settings['announcement_channel_id']}>"
                                                              if settings.get("announcement_channel_id") else "Not set"), inline=True)
        await interaction.response.edit_message(embed=embed, view=AwardAdminView(self.modules))
...
async def respond_with_admin_panel(interaction: discord.Interaction) -> None:
    ...
    await interaction.response.send_message(
        ...
        view=NativeAdminView(
            modules, str((stored or {}).get("channel_setup_mode") or "manual"),
            awards_available=_award_test_guild(interaction, bot),
        ),
        ephemeral=True,
    )
...
AWARD_NAME_LIMIT = 80
AWARD_DESCRIPTION_LIMIT = 500
AWARD_TASK_LIMIT = 100
AWARD_CITATION_LIMIT = 280
AWARD_MAX_REQUIREMENTS = 20


def _award_requirements(value: str | None) -> list[str]:
    if not value:
        return []
...
def _award_text_error(name: str, description: str, requirements: list[str]) -> str | None:
    if not name.strip() or len(name.strip()) > AWARD_NAME_LIMIT:
        return f"Award names must be 1-{AWARD_NAME_LIMIT} characters."
    if not description.strip() or len(description.strip()) > AWARD_DESCRIPTION_LIMIT:
        return f"Descriptions must be 1-{AWARD_DESCRIPTION_LIMIT} characters."
    if len(requirements) > AWARD_MAX_REQUIREMENTS:
        return f"An award can have at most {AWARD_MAX_REQUIREMENTS} requirements."
    if any(len(item) > AWARD_TASK_LIMIT for item in requirements):
        return f"Each task or contract name must be at most {AWARD_TASK_LIMIT} characters."
    return None


def _award_test_guild(interaction: discord.Interaction, bot: "GameAssistBot") -> bool:
    return bool(
        bot.settings.runtime_profile == "public"
        and bot.settings.award_test_guild_id
        and interaction.guild_id == bot.settings.award_test_guild_id
    )


async def _can_manage_awards(interaction: discord.Interaction, bot: "GameAssistBot") -> bool:
...
async def _enabled_award_settings(interaction: discord.Interaction, bot: "GameAssistBot") -> dict | None:
    if not _award_test_guild(interaction, bot):
        ...
        return None
    settings = await bot.cache.award_settings(interaction.guild_id or 0)
...
async def _announce_award(bot: "GameAssistBot", guild_id: int, user_id: int,
    ...
    try:
        ...
        logging.warning("Could not post award announcement in channel %s", channel_id)
        return False
...
async def reputation_submit_command(interaction: discord.Interaction, rep_giver: str, level: str,
    ...
    bot.track_reputation_verification(bot.process_reputation_submission(
...
@app_commands.describe(enabled="Turn the optional award system on or off", manager_role="Role allowed to manage awards")
async def award_configure_command(interaction: discord.Interaction, enabled: bool,
                                  manager_role: discord.Role | None = None) -> None:
    bot = interaction.client
    if not isinstance(bot, GameAssistBot) or not _award_test_guild(interaction, bot) or interaction.guild is None:
        await interaction.response.send_message("Awards are limited to the SC Companion testing Discord.", ephemeral=True)
        return
    if interaction.guild.owner_id != interaction.user.id:
        await interaction.response.send_message("Only the server owner can configure award managers.", ephemeral=True)
        return
    existing = await bot.cache.award_settings(interaction.guild.id)
    role_id = manager_role.id if manager_role else existing.get("manager_role_id")
    await bot.cache.save_award_settings(
        interaction.guild.id, enabled, role_id, interaction.user.id, existing.get("announcement_channel_id")
    )
    role_text = f" Managers: <@&{role_id}>." if role_id else " Only the server owner can manage awards."
    await interaction.response.send_message(
        f"Award system {'enabled' if enabled else 'disabled'}.{role_text}", ephemeral=True
    )
...
async def award_create_command(interaction: discord.Interaction, name: str, description: str,
    ...
    if error:
        await interaction.response.send_message(error, ephemeral=True)
        return
    if award_type.value == "tracker" and not tasks:
        await interaction.response.send_message("A tracked award needs at least one named task or contract.", ephemeral=True)
        return
    if award_type.value == "custom" and tasks:
        await interaction.response.send_message("Custom awards are granted directly and cannot have tracker requirements.", ephemeral=True)
        return
    existing = await bot.cache.award_definitions(interaction.guild_id or 0, active_only=False)
    if any(item["name"].casefold() == name.strip().casefold() for item in existing):
...
async def award_edit_command(interaction: discord.Interaction, award_id: int, name: str | None = None,
    ...
    if not isinstance(bot, GameAssistBot) or await _enabled_award_settings(interaction, bot) is None:
        return
    if not await _can_manage_awards(interaction, bot):
        await interaction.response.send_message("Only the server owner or configured award role can edit awards.", ephemeral=True)
        return
    award = await bot.cache.award_definition(interaction.guild_id or 0, award_id)
    if award is None:
        await interaction.response.send_message("That award was not found.", ephemeral=True)
        return
    tasks = _award_requirements(requirements) if requirements is not None else award["requirements"]
    next_name = name.strip() if name is not None else award["name"]
    next_description = description.strip() if description is not None else award["description"]
    error = _award_text_error(next_name, next_description, tasks)
    if error:
        await interaction.response.send_message(error, ephemeral=True)
        return
    if award["award_type"] == "tracker" and not tasks:
        await interaction.response.send_message("A tracked award must keep at least one requirement.", ephemeral=True)
        return
    existing = await bot.cache.award_definitions(interaction.guild_id or 0, active_only=False)
    if any(item["id"] != award_id and item["name"].casefold() == next_name.casefold() for item in existing):
        await interaction.response.send_message("An award with that name already exists.", ephemeral=True)
        return
...
@award_group.command(name="report", description="Report completion of one tracked task or contract.")
@app_commands.describe(award_id="Tracked award number", task="Exact task or contract name", citation="Brief proof or citation")
async def award_report_command(interaction: discord.Interaction, award_id: int, task: str, citation: str) -> None:
    bot = interaction.client
    if not isinstance(bot, GameAssistBot) or await _enabled_award_settings(interaction, bot) is None:
        return
    award = await bot.cache.award_definition(interaction.guild_id or 0, award_id)
    if award is None or not award["active"] or award["award_type"] != "tracker":
        await interaction.response.send_message("That active tracked award was not found.", ephemeral=True)
        return
    if not citation.strip() or len(citation.strip()) > AWARD_CITATION_LIMIT:
        ...
        return
    matched = next((item for item in award["requirements"] if item.casefold() == task.strip().casefold()), None)
    if matched is None:
        available = ", ".join(award["requirements"])
        await interaction.response.send_message(f"Use one of these exact requirements: {available}"[:1900], ephemeral=True)
        return
    approved = await bot.cache.approved_award_tasks(interaction.guild_id or 0, award_id, interaction.user.id)
    if matched.casefold() in approved:
        await interaction.response.send_message("That requirement is already approved for you.", ephemeral=True)
        return
    report_id = await bot.cache.submit_award_report(
        interaction.guild_id or 0, award_id, interaction.user.id, str(interaction.user), matched, citation.strip()
    )
    await interaction.response.send_message(
        f"Completion report `#{report_id}` submitted for **{award['name']}** — **{matched}**.", ephemeral=True
    )


@award_group.command(name="queue", description="Show completion reports waiting for review.")
async def award_queue_command(interaction: discord.Interaction) -> None:
    bot = interaction.client
    if not isinstance(bot, GameAssistBot) or await _enabled_award_settings(interaction, bot) is None:
        return
    if not await _can_manage_awards(interaction, bot):
        await interaction.response.send_message("Only configured award managers can view this queue.", ephemeral=True)
        return
    reports = await bot.cache.pending_award_reports(interaction.guild_id or 0)
    embed = discord.Embed(title="Award completion review queue", color=discord.Color.gold())
    embed.description = "No reports are waiting." if not reports else "Review with `/award review`."
    for report in reports[:10]:
        embed.add_field(
            name=f"#{report['id']} · {report['award_name']} · {report['task_name']}",
            value=f"<@{report['user_id']}> — {report['citation']}"[:1024], inline=False,
        )
    await interaction.response.send_message(embed=embed, ephemeral=True)


@award_group.command(name="review", description="Approve or reject a reported task completion.")
@app_commands.choices(decision=[app_commands.Choice(name="Approve", value="approved"),
                               app_commands.Choice(name="Reject", value="rejected")])
async def award_review_command(interaction: discord.Interaction, report_id: int,
                               decision: app_commands.Choice[str]) -> None:
    bot = interaction.client
    if not isinstance(bot, GameAssistBot) or await _enabled_award_settings(interaction, bot) is None:
        return
    if not await _can_manage_awards(interaction, bot):
        await interaction.response.send_message("Only configured award managers can review reports.", ephemeral=True)
        return
    result = await bot.cache.review_award_report(interaction.guild_id or 0, report_id, decision.value, interaction.user.id)
    if result is None:
        await interaction.response.send_message("That pending report was not found.", ephemeral=True)
        return
    completion = ""
...
@award_group.command(name="list", description="List available tracked and custom awards.")
async def award_list_command(interaction: discord.Interaction) -> None:
    bot = interaction.client
    if not isinstance(bot, GameAssistBot) or await _enabled_award_settings(interaction, bot) is None:
        return
    awards = await bot.cache.award_definitions(interaction.guild_id or 0)
    embed = discord.Embed(title="Available awards", color=discord.Color.blurple())
    embed.description = "No awards have been created." if not awards else None
...
@award_group.command(name="profile", description="Show awards earned by you or another member.")
async def award_profile_command(interaction: discord.Interaction, member: discord.Member | None = None) -> None:
    bot = interaction.client
    if not isinstance(bot, GameAssistBot) or await _enabled_award_settings(interaction, bot) is None:
        return
    target = member or interaction.user
    awards = await bot.cache.member_awards(interaction.guild_id or 0, target.id)
    embed = discord.Embed(title=f"Awards · {target.display_name}", color=discord.Color.green())
    embed.description = "No awards earned yet." if not awards else None
    for award in awards[:20]:
        embed.add_field(name=award["name"], value=f"{award['description']}\n*{award['citation']}*"[:1024], inline=False)

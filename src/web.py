...
async def guild_bot_configuration(guild_id: int, user=Depends(require_user)) -> dict[str, Any]:
    ...
    if awards_available:
        ...
        reputation_settings.pop("auto_verify", None)
...
async def create_award_announcement_channel(guild_id: int, user=Depends(require_user)) -> dict[str, Any]:
    await _award_dashboard_manager(guild_id, user)
    channels = await _discord_guild_channels(guild_id)
    category = next(
    ...
    for name, topic in channel_specs:
        ...
        channel = next(
            (item for item in channels if item["type"] in {0, 5} and item["name"] in aliases), None
        )
        if channel is None:
            channel = await _discord_api(
                "POST", f"/guilds/{guild_id}/channels", bot_token=_public_bot_token(),
                json_payload={"name": name, "type": 0, "parent_id": str(category["id"]), "topic": topic},
            )
            created = True
        elif channel.get("parent_id") != category["id"] or channel["name"] != name:
            channel = await _discord_api(
                "PATCH", f"/channels/{channel['id']}", bot_token=_public_bot_token(),
                json_payload={"name": name, "parent_id": str(category["id"]), "topic": topic},
            )
        award_channels[name] = channel
    award_channel = award_channels["award-announcements"]
    ...
    return {
        "status": "created" if created else "associated",
        "category_id": _snowflake(category["id"]),
...
async def _create_reputation_channels(guild_id: int, user: Any) -> dict[str, Any]:
    ...
    if review_queue is None:
        review_queue = await _discord_api(
            "POST", f"/guilds/{guild_id}/channels", bot_token=_public_bot_token(),
            json_payload={
                "name": "rep-review-queue", "type": 0, "parent_id": str(category["id"]),
        ...
        review_queue = await _discord_api(
            "PATCH", f"/channels/{review_queue['id']}", bot_token=_public_bot_token(),
            json_payload={
    ...
    panel_payload = {
        "embeds": [{
            ...
            "fields": [
                ...
                },
                {
                    ...
                    "value": "The configured reviewer role checks every screenshot and approves or denies it.",
                    "inline": False,
                },
            ],
        ...
        "components": [{
            "type": 1,
            "components": [{
                "type": 2, "style": 1, "label": "Submit Reputation",
                "custom_id": "reputation_application:start", "emoji": {"name": "📈"},
            }],
        }],
    }
    panel_key = f"guild:{guild_id}:reputation-submission-panel"
    panel_message_id = await state().cache.get(panel_key)
    panel_message = None
    if panel_message_id:
        try:
            panel_message = await _discord_api(
                "PATCH", f"/channels/{made['rep-submissions']['id']}/messages/{panel_message_id}",
                bot_token=_public_bot_token(), json_payload=panel_payload,
            )
        except HTTPException as error:
            if error.status_code != 404:
                raise
    if panel_message is None:
        panel_message = await _discord_api(
            "POST", f"/channels/{made['rep-submissions']['id']}/messages",
            bot_token=_public_bot_token(), json_payload=panel_payload,
        )
        await state().cache.set(panel_key, int(panel_message["id"]), 315360000)

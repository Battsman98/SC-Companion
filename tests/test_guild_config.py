...
def test_management_panel_is_available_to_discord_server_managers() -> None:
    ...
    assert "renderFeatureAssignmentsByCategory" in javascript
    ...
    assert 'name="auto_verify"' not in javascript
    assert "Automatically verify screenshots" not in javascript
    assert "configured reviewer role for approval or denial" in javascript
...
def test_reputation_submissions_use_a_public_panel_and_private_review_queue() -> None:
    ...
    assert '"Private reviewer approval"' in provision_source

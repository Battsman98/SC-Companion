...
def test_public_sc_companion_can_publish_examples_in_the_support_guild() -> None:
    ...
    assert 'item.name == "SC Companion"' in support_resources_source
    assert "await channel.delete" in support_resources_source
    assert "await retired.delete" in support_resources_source
    assert "must never" in support_resources_source

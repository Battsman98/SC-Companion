...
async function loadGuildBotConfiguration(guildId) {
  ...
  try {
    ...
    const syncAwardAutoGrant = (form) => {
      ...
      const control = form.querySelector("[data-award-auto-grant]");
      if (!control) return;
      control.classList.toggle("control-disabled", !requirements.length);
      form.elements.auto_grant.disabled = !requirements.length;
      if (!requirements.length) form.elements.auto_grant.checked = false;
    };
    if (awardCreateForm) {
...
function renderReputationFeature(config) {
  ...
  return `<div class="bot-feature-row" data-reputation-feature data-reviewer-role-id="${escapeAttribute(settings.reviewer_role_id || "")}">
    <label class="bot-module-copy"><input type="checkbox" data-reputation-feature-enabled ${settings.enabled ? "checked" : ""}><span><strong>Reputation Progress Tracker</strong><small>Tracks all 32 current reputation ladders plus each member's monthly message and voice activity. Every reputation giver has a stable, unique two-color ribbon on the on-demand <code>/progress</code> card, without creating reputation roles or a permanent profile message.</small><small>Screenshot applications stay private for reviewers. ${settings.submission_channel_id ? "Private application queue configured." : "No private application queue is associated yet."}</small></span></label>
  </div>`;
}
...
function renderReputationManagement(config) {
  ...
  return `<section class="bot-award-management" data-reputation-management data-guild-id="${config.guild.id}" data-enabled="${settings.enabled ? "true" : "false"}">
    <div class="section-heading"><p class="guide-kicker">TESTING DISCORD</p><h3>Reputation Progress Tracker</h3><p>SC Companion stores approved ranks for all 32 current reputation ladders without consuming Discord role slots. Members submit a giver, level, and screenshot through <code>/rep submit</code>; the application is visible only to reviewers, and approval updates the member's saved rank for that giver. <code>/progress</code> generates an on-demand profile card with a unique two-color ribbon for every giver, plus the current month's message count, voice time, and active days. It does not maintain a permanent profile message.</p></div>
...
async function editDashboardAward(event) {
  ...
  const form = event.currentTarget;
  const requirements = awardRequirements(form.elements.requirements.value);
  await awardDashboardRequest(form, `/${form.dataset.awardId}`, "PUT", { name: form.elements.name.value, description: form.elements.description.value, requirements, active: form.elements.active.checked, auto_grant: Boolean(requirements.length && form.elements.auto_grant.checked) });
}

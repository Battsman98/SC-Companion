...
function renderReputationManagement(config) {
  ...
  return `<section class="bot-award-management" data-reputation-management data-guild-id="${config.guild.id}" data-enabled="${settings.enabled ? "true" : "false"}">
    <div class="section-heading"><p class="guide-kicker">TESTING DISCORD</p><h3>Reputation Progress Tracker</h3><p><code>/activity</code> works without reputation and shows how long a member has been in the Discord plus the current month's message count, voice time, and active days. SC Companion also stores approved ranks for all 32 current reputation ladders without consuming Discord role slots. Members submit proof through the application panel or <code>/rep-submit</code>, and <code>/rep</code> generates the combined activity and reputation embed on demand.</p></div>
    <form data-reputation-settings-form class="tool-card award-form reputation-settings-form">
      ...
      <p class="form-note">Every screenshot is sent privately to the configured reviewer role for approval or denial.</p>
...
const discordConsole = document.querySelector("[data-discord-console]");
...
async function openDiscordConsole() {
...
function renderAwardManagement(config) {
  const awards = config.awards || { settings: {}, definitions: [], pending_reports: [], grants: [], roles: [] };
  ...
  return `<section class="bot-award-management" data-award-management data-guild-id="${config.guild.id}" data-enabled="${settings.enabled ? "true" : "false"}">
    ...
    <details class="tool-card award-create-section award-create-disclosure">
      <summary><span class="award-create-summary"><span class="award-create-emblem" aria-hidden="true">🏅</span><span><span class="guide-kicker">AWARDS / NEW</span><strong>Create New Award</strong><small>Create an award as a custom recognition or requirement-based achievement.</small></span></span><span class="award-disclosure-chevron" aria-hidden="true">⌄</span></summary>
    ...
    <section class="award-existing-section"><div class="award-existing-heading"><div><p class="guide-kicker">AWARDS / MANAGE</p><h4>Existing Awards</h4><p>Edit award details, requirements, and availability using the same layout as award creation.</p></div><span class="award-count-badge">${definitions.length}</span></div>
      <div class="award-existing-list">${definitions.length ? definitions.map((award) => `<details class="tool-card award-edit-disclosure"><summary><span class="award-summary-title"><span class="award-summary-medal" aria-hidden="true">🏅</span><span>${escapeHtml(award.name)}</span></span><span class="award-summary-action"><span>${award.active ? "Active" : "Inactive"}</span><span class="award-disclosure-chevron" aria-hidden="true">⌄</span></span></summary><form data-award-edit-form data-award-id="${award.id}" class="award-form award-create-card award-edit-card">
        <div class="award-edit-heading"><div><span class="award-type-badge">${escapeHtml(award.award_type === "tracker" ? "Tracked award" : "Custom award")}</span><h5>${escapeHtml(award.name)}</h5></div><span class="award-status-badge ${award.active ? "is-active" : "is-inactive"}">${award.active ? "Active" : "Inactive"}</span></div>
    ...
    <div class="tool-card award-card award-review-card"><div class="award-card-heading"><div><p class="guide-kicker">AWARDS / REVIEW</p><h4>Completion reports</h4><p>Members submit award requests from the Discord Award Panel. Review them here.</p></div><span class="award-count-badge">${(awards.pending_reports || []).length}</span></div>${(awards.pending_reports || []).length ? awards.pending_reports.map((report) => `<div class="bot-management-actions"><span><strong>${escapeHtml(report.award_name)}</strong><br>${escapeHtml(report.user_name)} — ${escapeHtml(report.task_name)}<br><small>${escapeHtml(report.citation)}</small></span><button type="button" data-award-review data-report-id="${report.id}" data-decision="approved">Approve</button><button type="button" data-award-review data-report-id="${report.id}" data-decision="rejected">Reject</button></div>`).join("") : '<div class="state award-empty-state">No reports are waiting.</div>'}</div>
    <details class="tool-card award-log-card"><summary><span><span class="guide-kicker">AWARDS / HISTORY</span><strong>Award Log</strong><small>Issued awards and recipients</small></span><span class="award-summary-action"><span>${(awards.grants || []).length} issued</span><span class="award-disclosure-chevron" aria-hidden="true">⌄</span></span></summary><div class="award-log-list">${(awards.grants || []).length ? awards.grants.map((grant) => `<article class="award-log-entry"><div><strong>${escapeHtml(grant.award_name)}</strong><span>${escapeHtml(grant.recipient_name)} · <code>${escapeHtml(String(grant.recipient_id))}</code></span></div><p>${escapeHtml(grant.citation)}</p><small>Issued ${escapeHtml(dateTime(grant.granted_at))} · Manager <code>${escapeHtml(String(grant.granted_by))}</code></small></article>`).join("") : '<div class="state award-empty-state">No awards have been issued yet.</div>'}</div></details>
    <form data-award-grant-form class="tool-card award-form award-grant-card"><div class="award-card-heading"><div><p class="guide-kicker">AWARDS / GRANT</p><h4>Grant an award</h4><p>Assign an active award directly to a Discord member.</p></div></div>
      <div class="award-field-grid"><label><span>Award</span><select name="award_id" required><option value="">Choose an award...</option>${activeAwards.map((award) => `<option value="${award.id}">${escapeHtml(award.name)}${award.requirements?.length ? " (requirements tracked)" : ""}</option>`).join("")}</select></label><label><span>Discord member ID</span><input name="member_id" inputmode="numeric" pattern="[0-9]+" placeholder="Example: 123456789012345678" required><small>Enable Developer Mode in Discord to copy an ID.</small></label></div>
      ...
      <div class="award-form-actions"><button type="submit" ${activeAwards.length ? "" : "disabled"}>Grant and Announce</button><span class="form-note" data-award-status>${activeAwards.length ? "" : "Create an active award first."}</span></div>
    </form>
  </section>`;
}
...
feedbackForm?.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    clearFeedbackPreviews();
    closeFeedbackModal();
    const successMessage = document.querySelector("#feedbackSuccessMessage");
    const ticketLink = document.querySelector("[data-feedback-ticket-link]");
    if (successMessage) successMessage.textContent = "Your report is now in the Discord feedback forum. You can continue the conversation there and receive status updates.";
    if (ticketLink) { ticketLink.href = payload.ticket_url || "#"; ticketLink.hidden = !payload.ticket_url; }
    openFeedbackSuccess();
  } catch (error) {

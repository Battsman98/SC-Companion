    ...
    outputs.botManagement.innerHTML = `<form data-bot-management-form data-guild-id="${config.guild.id}">
      ...
      <div class="bot-module-list">
        ${config.modules.map((module) => `<div class="bot-module-row" data-module-key="${escapeAttribute(module.key)}">
          ...
          <div><label>Command channel${renderDiscordChannelPicker(config.channels, module.channel_id)}</label>${module.key === "trade_tools" ? `<label>Marketplace forum${renderDiscordChannelPicker(config.channels, module.resource_channel_id, { forum: true })}</label>` : ""}</div>
        </div>`).join("")}
      </div>
